...
*Python · scikit-learn · FastAPI · MLflow · DVC · DagsHub · Docker · GitHub Actions · Google Cloud Run*
...
- **Amount realism**: per-category lognormal amount distributions, with shapes informed by the per-category amount statistics of a 1.3M-row credit card transactions dataset (see [Data sources](#data-sources))
...
Logistic Regression won the comparison — a real, explainable outcome (linear models generalize better on high-dimensional sparse TF-IDF features, while Random Forest overfits noisy text). Test slightly exceeding validation is sampling noise on 1,000-row evaluation sets; the training notebook includes a **robustness check** that refits the pipeline across 5 different train/validation splits: validation accuracy averages **0.775 with a 0.030 spread** depending on the split alone, while test accuracy stays stable at **0.791 with a 0.008 spread** — so the val/test gap is sampling variation, not leakage.

The confusion matrix shows the errors are exactly where the data is genuinely ambiguous — education↔shopping (book purchases), shopping↔travel, and the healthcare column collecting generic vendor payments:

![Confusion matrix — Logistic Regression, validation set](docs/images/confusion_matrix.png)
...
![MLflow experiment history on DagsHub](docs/images/dagshub_experiments.png)
...
The dataset is generated by [`data/generate_dataset.py`](data/generate_dataset.py), but its design draws on two public Kaggle datasets, which were analyzed while building the generator:

- [Indian Banking Transaction Text Dataset](https://www.kaggle.com/datasets/coderanand/indian-banking-transaction-text-dataset) by coderanand (Apache 2.0) — inspired the style and part of the vocabulary of the merchant phrases (Indian bank-statement descriptions: Swiggy, HDFC EMI, IRCTC, etc.)
- [Credit Card Transactions Dataset](https://www.kaggle.com/datasets/priyamchoksi/credit-card-transactions-dataset) by Priyam Choksi (1.3M Sparkov-simulated transactions) — its per-category amount distributions informed the generator's per-category amount parameters (skew, spread, outlier behavior)

No rows were copied from either dataset; both ultimately proved unsuitable for direct training (the first has a closed ~28-phrase vocabulary, the second's simulated merchant names carry no text signal) — which is what motivated building the generator instead.
